...
public:
    ...
    static void* flush_log_thread(void *args) //异步日志记录函数，也是有pthread_create()的线程函数
    {
...
private:
    ...
    bool m_is_async;                  //是否同步标志位，默认为false
    locker m_mutex;
};

    

    #if defined(SYNLOG) || defined(ASYNLOG)
        #if DEBUG_VERSION
